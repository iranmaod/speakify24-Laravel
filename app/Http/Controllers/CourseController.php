...
class CourseController extends Controller
{
    ...
    public function myCourses(Request $request)
    {
        ...
        $courses = DB::table('courses')
                    ...
                    ->groupBy('appointments.course_id')
                    ->get();
    ...
    public function courseView($course_slug = '', Request $request)
    {
        ...
        $lecturesmedia = array();
        if (\Auth::user()) {
            $coursecurriculum = $this->model->getcurriculuminfo($course->id,\Auth::user()->id);
        ...
        $prices = CoursePrices::where('course_id', $course->id)->get();
        
        return view('site.course.view', compact('course', 'curriculum_sections', 'lectures_count', 'videos_count', 'video', 'course_breadcrumb', 'is_curriculum', 'prices', 'lecturesmedia'));
    }
    ...
    public function checkout($course_slug = '', $p_id = '', $instructor_id = '', $time = '', Request $request)
    {
        $course_breadcrumb = Session::get('course_breadcrumb');
        $course = Course::where('course_slug', $course_slug)->first();
        $price = CoursePrices::where('id', $p_id)->first();
        ...
        return view('site.course.checkout', compact('course', 'price', 'instructor', 'instructor_id', 'time', 'course_breadcrumb', 'coupon'));
    }
    ...
    public function myAppointments(Request $request)
    {
        ...
        $appointments = \DB::table('appointments')
                            ->select('appointments.*', 'courses.course_title', 'instructors.first_name', 'instructors.last_name')
                            ->leftJoin('instructors', 'appointments.instructor_id', '=', 'instructors.user_id')
                            ->leftJoin('courses', 'appointments.course_id', '=', 'courses.id')
                            ->where('appointments.user_id', $user->id)
                            ->get()
        ;

        return view('site.course.my-appointments', compact('appointments', 'user'));
    }
    ...
    public function myListing(Request $request)
    {
        ...
        $appointments = \DB::table('appointments')
                            ->select('appointments.*', 'instructors.first_name', 'instructors.last_name', 'courses.course_title')
                            ->leftJoin('instructors', 'appointments.instructor_id', '=', 'instructors.user_id')
                            ->leftJoin('courses', 'appointments.course_id', '=', 'courses.id')
                            ->where('appointments.user_id', $user->id)
        ...
        return view('site.course.listing', compact('appointments', 'user'));
    }
    ...
    public function mytransactions(Request $request) {
        ...
        if($request->has('search') || $request->has('startdate') || $request->has('enddate')){
            ...
            $transactions = \DB::table('transactions')
                            ->select('transactions.*', 'courses.course_title')
            ...
            if (!empty($search)) {
                $transactions->where(function ($q) use ($search) {
                    $q->where('courses.course_title', 'LIKE', '%' . $search . '%');
                });
            }
        ...
        return view('site.course.mytransactions', compact('transactions', 'user'));
    }

    public function canceltransaction($t_id, $id, $type = '')
    {
        if (!$id) {
            return back();
        }
        ...
        try {
            ...
            if ($plan) {
                ...
                if ($plan->toArray()['state'] == 'DELETED') {
                    return $this->return_output('flash', 'success', 'Plan already deleted.', 'mytransactions', '200');
                } else {
                ...
                if ($type == 'admin') {
                    return $this->return_output('flash', 'success', 'You request has been processed.', 'admin/member_subscriptions', '200');
                } else {
                    return $this->return_output('flash', 'success', 'You request has been processed.', 'mytransactions', '200');
                }
            } else {
                return $this->return_output('flash', 'success', 'Plan already deleted.', 'mytransactions', '200');
            }
    ...
    public function addAppointment(Request $request)
    {
        ...
        $amount = 0;
        $instructor = Instructor::where('user_id', $data['instructor_id'])->first();
		...
		$appointment->title = 'New ' . $user->id . ' ' . $data['instructor_id'];
		$appointment->user_id = $user->id;
		$appointment->course_id = $data['course_id'];
		$appointment->transaction_id = $data['transaction_id'];
		$appointment->instructor_id = $data['instructor_id'];
    ...
    public function cancelAppointment($id) {
        ...
        ///////sending email to instructor after cancelling
        $appointment->instructor = User::find($appointment->instructor_id);
        $appointment->student = User::find($appointment->user_id);
        $data = $appointment;
        ...
        return redirect('/listing')->with('success', 'Appointment cancelled successfully.');
    }
}
