...
class CourseController extends Controller
{
    ...
    public function myCourses(Request $request)
    {
        ...
        return view('admin.course.my-courses', compact('courses'));
    }
/*
    ...
    public function courseView($course_slug = '', Request $request)
    {
        ...
        return view('admin.course.view', compact('course', 'curriculum_sections', 'lectures_count', 'videos_count', 'video', 'course_breadcrumb', 'is_curriculum'));
    }
    ...
    {   
        ...
        return view('admin.course.learn', compact('course', 'curriculum_sections', 'lectures_count', 'videos_count', 'video', 'course_breadcrumb', 'is_curriculum', 'course_rating', 'students_count'));
    }
    ...
    public function courseList($course_slug = '', Request $request)
    {
        ...
        return view('admin.course.list', compact('courses', 'categories', 'instruction_levels'));
    }
    ...
    public function checkout($course_slug = '', Request $request)
    {
        ...
        return view('admin.course.checkout', compact('course', 'course_breadcrumb'));
    }
...
*/
    ...
    {   
        ...
        return view('admin.course.create_info', compact('course', 'categories', 'languages', 'instruction_levels', 'instructors'));
    }
    ...
    {   
        ...
        if( $course ) {
            \DB::table('course_taken')->where('course_id', '=', $course_id)->delete();
            \DB::table('course_videos')->where('course_id', '=', $course_id)->delete();
            \DB::table('course_ratings')->where('course_id', '=', $course_id)->delete();
            \DB::table('course_progress')->where('course_id', '=', $course_id)->delete();
            $course->delete();
        }
        return redirect()->route('admin.course.list')->with('success', 'Course Deleted!');
    }
    ...
    {
        $courses = Course::select("instructor_id")->get()->toArray();
        $courseInstructorId = array();

        $course_id = $request->input('course_id');
        if ($course_id) {
            $courseData = Course::select("instructor_id")->where('id', $course_id)->first()->toArray();

            foreach($courses as $course) {
                foreach($course['instructor_id'] as $instructor_id) {
                    if ( !in_array($instructor_id, $courseData['instructor_id']) ) {
                        $courseInstructorId[$instructor_id] = $instructor_id;
                    }
                }
            }
        }
        ...
        if ($course_id) {
            $instructor_id = [
                'required',
                'array',
                'unique:courses,instructor_id,'.$course_id,
            ];
        } else {
            $instructor_id = [
                'required',
                'array',
                'unique:courses,instructor_id',
            ];
        }

        $diff = array_intersect($data['instructor_id'], $courseInstructorId);
        ...
        if ($validator->fails()) {
            ...
            $validator->errors()->add('instructor_id', 'Instructor is already in use!');
            return $this->return_output('error', 'error', $validator, 'back', '422');
        }
    ...
    public function instructorCourseVideoSave(Request $request)
    {
        ...
        $courseVideos->video_title = 'raw_'.time().'_'.str_replace("https://www.youtube.com/watch?v=", "", $request->input("url"));
        $courseVideos->video_name = $request->input("url");
        ...
        $courseVideos->created_at = time();
        $courseVideos->updated_at = time();

        if($courseVideos->save()){
            $course->course_video = $courseVideos->id;
            $course->save();

            return $this->return_output('flash', 'success', 'Course video updated successfully', 'admin/course/video/'.$course_id, '200');
        }else{
            return $this->return_output('flash', 'error', 'Course video error', 'admin/course/video/'.$course_id, '200');
        }
    }
    ...
    public function coursePrices($course_id = '',Request $request)
    {   
        $course = Course::find($course_id);
        $prices = $this->model->getPrices($course_id);

        return view('admin.course.create_prices', compact('course', 'prices'));
    }

    public function coursePricesSave(Request $request)
    {
        $course_id = $request->input('course_id');
        $course = Course::find($course_id);
        if (count($request->input('hours')) == 1 && $request->input('hours')[0] == '') {
            $return_data = array(
                'status' => false,
            );
            return $this->return_output('flash', 'error', 'Course prices error', 'admin/course/prices/'.$course_id, '200');
        }
        $excludedPrice = CoursePrices::where('course_id', $course_id)
                            ->whereNotIn('id', $request->input('id'))->get();

        if ( $excludedPrice ) {
            foreach ( $excludedPrice as $ep ) {
                $ep->delete();
            }
        }

        foreach($request->input('hours') as $key => $value) {
            $discount = $request->input('price')[$key];
            if ($value == '' || $discount == '' || $value == '0' || $discount == '0') {
                return $this->return_output('flash', 'error', 'Number of Hours or Price can\'t be null.', 'admin/course/prices/'.$course_id, '200');
            }

            $CoursePrices = new CoursePrices;
            if ( isset($request->input('id')[$key]) ) {
                $id = $request->input('id')[$key];
                if ($id != '0') {
                    $CoursePrices = CoursePrices::find($id);
                }
            }

            $CoursePrices->created_at = date("Y-m-d H:i:s", time());
            $CoursePrices->hours = $value;
            $CoursePrices->price = $discount;
            $CoursePrices->course_id = $course_id;
            $CoursePrices->updated_at = date("Y-m-d H:i:s", time());
            $CoursePrices->save();

            if ( isset($request->file('image')[$key]) ) {
                $photo = $request->file('image')[$key];
                if ($photo) {
                    $photoSaveAsName = time() . "-image." . $photo->getClientOriginalExtension();

                    $upload_path = 'storage/app/public/course_prices/'.$CoursePrices->id.'/';
                    $photo_url = $upload_path . $photoSaveAsName;
                    $success = $photo->move($upload_path, $photoSaveAsName);
                    $photo_url = 'course_prices/'.$CoursePrices->id.'/' . $photoSaveAsName;
                    $CoursePrices->image = $photo_url;
                    $CoursePrices->save();
                }
            }
        }

        return $this->return_output('flash', 'success', 'Course prices updated successfully', 'admin/course/prices/'.$course_id, '200');
    }
}
