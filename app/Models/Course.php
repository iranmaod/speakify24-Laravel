...
class Course extends Model
{
	...
	{
		$instructors = $this->getInstructorRecords();

		$instructor_name = '';
		foreach($instructors as $instruct) {
			$instructor_name .= $instruct->display_name . '<br />';
		}
		return $instructor_name;
	}

	public function getInstructorRecords()
	{
		$ids = array();

		foreach($this->instructor_id as $ins_id) {
			$ids[$ins_id] = $ins_id;
		}

		$instructors = \DB::table('instructors')
			->select('user_id', 'instructor_slug', \DB::raw("CONCAT(first_name, ' ', last_name) AS display_name"))
			->whereIn('user_id', $ids)
			->get()
		;
  ...
  public function getFilebyid($file_id=''){
    ...
    public function getPrices($course_id='')
    {
       return \DB::table('course_prices')->where('course_id', '=', $course_id)->get(); 
    }
