...
if (count($appointments) > 0) {
	foreach ($appointments as $appointment) {
		...
		\Mail::send([], [
			...
			'date' => $date,
			'time' => $time
		], function ($message) use($data, $messageHeadCSS, $messageBody, $messageBodyDate, $messageBodyTime, $messageRemainFooter, $date, $time){
			...
			$mailBody .= '<p>Am '.$date.' um '.$time.' hast du mit '.$data->student->first_name.' '.$data->student->last_name.' Unterricht.<br></p>';
		...
		\Mail::send([], [
			...
			$mailBody .= $messageBody;
			$mailBody .= 'wir wollen dich daran erinnern, dass dein Unterricht bald beginnt.<br></p>';
			$mailBody .= '<p>Am '.$date.' um '.$time.' hast du mit '.$data->instructor->first_name.' '.$data->instructor->last_name.' Unterricht.<br></p>';
			$mailBody .= '<p>Logge dich dazu bitte unter www.speakify24.de ein und klicke auf das blaue Kamerasymbol.<br></p>';
			$mailBody .= '<p>Viel Spaß beim Unterricht!<br></p>';
			$mailBody .= '<p>Dein Speakify24 Team';
			$mailBody .= $messageBodyDate;
