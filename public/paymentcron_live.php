	...
	foreach ($transactions as $transaction) {
		...
		try {
			...
			if ($bill_date == date('Y-m-d')) {
				...
				$transaction->save();
				mail("talhauos1@gmail.com","Payment successful",'New transaction ('.$newtransaction->id.') added.' . date('Y-m-d h:i:s').'Old transaction id '.$transaction->id);
				file_put_contents($log_file_data, 'New transaction ('.$newtransaction->id.') added.' . date('Y-m-d h:i:s') . "\n", FILE_APPEND);
			}
		} catch (Exception $ex) {
			mail("talhauos1@gmail.com","Payment Failed Exception",'Previous transaction id is  '. $transaction->id .' with order details' .$orderDetails . 'Exception error msg' . $ex->getMessage() . date('Y-m-d h:i:s'));
			file_put_contents($log_file_data, $ex->getMessage() . date('Y-m-d h:i:s') . "\n", FILE_APPEND);
		}
	}
	exit;
} else {
	mail("talhauos1@gmail.com","No transaction",'No transaction found');
