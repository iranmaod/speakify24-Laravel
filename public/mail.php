...
$start = date('Y-m-d H:i:s', strtotime('+27 hours'));

$appointments = Appointment::where('startdate', '<=', $start)
                            ->where('startdate', '>=', date("Y-m-d H:i:s", strtotime("+26 hours")))
							->where('status', 1)
							->get();
...
if (count($appointments) > 0) {
    foreach ($appointments as $appointment) {
        ...
        Mail::to($data->instructor->email)->send(new TeacherReminder($data));
        ...
        Mail::to($data->student->email)->send(new StudentReminder($data));
        $log_filename = "log";
