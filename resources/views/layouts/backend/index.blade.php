...
<body>
  <nav class="site-navbar navbar navbar-default navbar-fixed-top navbar-mega navbar-expand-md"
    ...
    <div class="navbar-header">
      ...
      <div class="navbar-brand navbar-brand-center">
        <img class="navbar-brand-logo" src="{{ asset('backend/assets/images/b_logo.png') }}" title="Speakify">
      </div>
          ...
          <li class="nav-item">
            @if(Auth::check() && Auth::user()->hasRole('instructor'))
              ...
              {{-- <div id="messagelink" style="display:inline-block;margin-right: 10px;"> --}}
                  <a class="nav-link remining_lectures" title="Messages" href="{{ url('/messages') }}">
                    <img style="width:22px;" src="{{ asset('frontend/img/menu-chat.png') }}" />
                    <span style="position:absolute;background-color:#162d74;color:#fff;height:20px;width:20px;text-align:center;border-radius:10px;margin-top:-14px;font-size:10px;line-height:20px;margin-left:-5px;">{{ count($messages) }}</span>
                  </a>
              {{-- </div> --}}
            @endif
          </li>
  ...
  <footer class="site-footer">
    <div class="site-footer-legal">© {{ date('Y') }} <a href="javascript:void(0)">Speakify</a></div>
  </footer>
  <!-- Core  -->
   ...
   $.ajaxSetup({
    ...
    (function(document, window, $) {
      ...
      $(document).ready(function() {
        ...
        var dcoff = document.cookie.split(';');
        for(var i = 0; i < dcoff.length; i++) {
          var c = dcoff[i];
          while (c.charAt(0) == ' ') {
            c = c.substring(1);
          }
          if (c.indexOf('offset') == 0) {
            var cv = c.split("=");
            console.log(cv);
            console.log(cv[1] != offset);
            if (cv[1] != offset) {
