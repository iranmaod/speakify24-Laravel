  ...
  <div class="panel-body">
    ...
    <form method="POST" action="{{ route('admin.course.info.save') }}" id="courseForm">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
            <?php
                ...
                if ($course->instructor_id != '') {
                    $courseInstructors = $course->instructor_id;
                }
            ?>
            <label class="form-control-label">Instructor <span class="required">*</span></label>
            <select class="form-control" name="instructor_id[]" multiple>
                <option value="">Select</option>
                @foreach($instructors as $instructor)
                    <option value="{{ $instructor->id }}"
                    @if($instructor->id == old('instructor_id')){{ 'selected="selected"' }}@elseif(in_array($instructor->id, $courseInstructors)){{ 'selected="selected"' }}@endif>
        ...
        <div class="form-group col-md-4">
            <label class="form-control-label">Language <span class="required">*</span></label>
            ...
            @if ($errors->has('language_id'))
                <label class="error" for="language_id">{{ $errors->first('language_id') }}</label>
            @endif
        </div>
        ...
        <div class="form-group col-md-4">
            <label class="form-control-label">Duration Type <span class="required">*</span></label>
            <select class="form-control" name="type">
                <option value="minutes"
                @if('minutes' == $course->type){{ 'selected' }}@endif>
                    Minutes
                </option>
                <option value="hours"
                @if('hours' == $course->type){{ 'selected' }}@endif>
                    Hours
                </option>
            </select>
