	...
	<div class="panel">
        <div class="panel-heading">
            <div class="panel-title">
              <!-- <a href="{{-- route('appointment.create') --}}" class="btn btn-success btn-sm"><i class="icon wb-plus" aria-hidden="true"></i> Add Appointment</a> -->
				...
				<tbody>
					...
					@foreach($appointments as $appointment)
						...
						<tr>
							...
							<!-- <td>{{-- $appointment->lecture_title --}}</td> -->
							<td>{{ $appStart }}</td>
							<td>{{ $appEnd }}</td>
							...
							<td>
								@if(strtotime('+2 day', strtotime($appointment->startdate)) > time() && $appointment->status != 3)
									<a href="{{ route('appointment.destroy', $appointment->id) }}" class="btn btn-sm btn-danger">Cancel</a>
								@endif
								@if(!$appointment->start && time() <= strtotime($appointment->enddate))
								...
								@elseif($appointment->status == '1' && $appointment->start && time() <= strtotime($appointment->enddate))
								<form method="POST" action="{{ route('joinmeeting') }}" accept-charset="UTF-8" class="">
									@csrf
									<input name="appointment_id" type="hidden" value="{{$appointment->id}}">
									<button class="btn btn-success btn-sm" type="submit" style="width: 100%;"> 
