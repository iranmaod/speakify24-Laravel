            ...
            <div class="breadcrumb-container">
                <div class="container">
                  <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="@if($course_breadcrumb) {{ $course_breadcrumb }} @else {{ route('course.list') }} @endif">
                        {{ __('booking.courselist') }}
                    </a></li>
        ...
        <article class="container mt-4">
            <div class="row">
               <div class="col-xl-7 offset-xl-3 col-lg-8 offset-lg-2 col-md-10 offset-md-1">
                    <h6 class="underline-heading mb-4">
                        {{ __('booking.confirmPurchase') }}
                    </h6>
                    
                    @php
                        $priceINT = $price->price;
                        if($coupon) {
                            if ($coupon->type == 'flat') {
                                $priceINT = $price->price - $coupon->discount;
                            } else {
                                $priceINT = $price->price - ($price->price * $coupon->discount / 100);
                            }
                            if ( $priceINT < 0 ) {
                                $priceINT = 0;
                            }
                        ...
                        <div class="col-xl-9 col-lg-9 col-md-9 col-sm-9 col-8">
                            ...
                            <div class="instructor-clist mb-0 mt-1 d-sm-block d-none">
                                <div class="ml-1">
                                    ...
                                    <span>{{ __('booking.category') }} <b>{{ $course->category->name }}</b></span>
                                </div>
                            </div>
                            <div class="instructor-clist mb-0 mt-1">
                                <div>
                                    <i class="fa fa-chalkboard-teacher"></i>&nbsp;
                                    <span>{{ __('booking.createdby') }} <b>{{ $instructor->first_name.' '.$instructor->last_name }}</b></span>
                                </div>
                            </div>
                            @php $course_price = $course->price == 0.00 ? 'Free' : config('config.default_currency').$course->price; @endphp
                            <h6 class="c-price-checkout">€ {{ $priceINT }}</h6>
                        </div>
                    ...
                    <input type="hidden" name="course_p_id" value="{{ $price->id }}">
                    ...
                    <div class="form-group mt-4">
                        <button type="submit" class="btn btn-lg btn-block social-btn facebook-btn">
                            <div class="row">
                                <div class="col-3">
                                    <i class="@if($price->price == 0.00) fas fa-cart-arrow-down @else fab fa-paypal  @endif float-right"></i>
                                </div>
                                <div class="col-9">
                                    <span>
                                    @if($price->price == 0.00)
