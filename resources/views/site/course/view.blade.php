...
<div class="container-fluid p-0 home-content">
    <!-- banner start -->
    <div class="subpage-slide-blue bgcoverimg course-bg">
        <div class="container">
            <h1>Course</h1>
        </div>
    </div>
    ...
    <!-- breadcrumb end -->
    <?php $insturctors = \App\Models\Course::find($course->id)->getInstructorRecords(); ?>
    <div class="container">
        <div class="row mt-4">
            <!-- course block start -->
            <div class="col-xl-9 col-lg-9 col-md-8">
                <div class="cv-course-container">
                    ...
                    <div class="instructor-clist  m-0">
                        ...
                        <div class="row">
                            <?php foreach($insturctors as $ins) { ?>
                                <div class="col-md-6 techertitle-Main p-2 m-0">
                                    <i class="fa fa-chalkboard-teacher"></i>&nbsp;
                                    <a href="{{ route('instructor.view', $ins->instructor_slug) }}" style="float:none;color:#787887;"><b class="col-md-3 p-2">{!! $ins->display_name !!}</b></a>
                                    <a href="{{ route('choose.plan', ['course_id' => $course->id, 'instructor_id' => $ins->user_id, 'type' => '1']) }}" class="btn btn-sm login-page-button"><span>BOOK PLAN</span></a>
                                    <a href="{{ route('choose.plan', ['course_id' => $course->id, 'instructor_id' => $ins->user_id, 'type' => '2']) }}" class="btn btn-sm mr-2 login-page-button"><span>BOOK HOURLY</span></a>
                                </div>
                            <?php } ?>
                        </div>
                    </div>
                    ...
                    <!-- <div class="instructor-clist  m-0">
                        ...
                        <div class="row">
                            <?php //foreach($prices as $pr) { ?>
                                <div class="col-md-4 techertitle-Main p-2 m-0">
                                    <i class="fa fa-chalkboard-teacher"></i>&nbsp;
                                    <b class="col-md-3 p-2">{{--$pr->hours--}} hours for ${{--$pr->price--}}</b>
                                    <a href="{{-- route('course.checkout', ['course_slug' => $course->course_slug,  'p_id' => $pr->id]) --}}" class="btn btn-sm login-page-button"><span>CHOOSE PACKAGE</span></a>
                                </div>
                            <?php //} ?>
                        </div>
                    </div> -->
                    <div class="row cv-header mt-5">
                                    ...
                                    <article  class="preview-time">
                                        <span>
                                        	@if($curriculum_lecture->media_type == 2)
                                                @if(\Auth::user())
                                                    <a href="{{ Storage::url('course/'.$course->id.'/'.$lecturesmedia[$section_id][$curriculum_lecture->lecture_quiz_id][0]->file_name.'.'.$lecturesmedia[$section_id][$curriculum_lecture->lecture_quiz_id][0]->file_extension) }}">{!! $lecturesmedia[$section_id][$curriculum_lecture->lecture_quiz_id][0]->file_title !!}</a>
                                        		@else
